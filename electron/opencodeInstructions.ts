 ...
 *
 * opencode reads AGENTS.md automatically for project instructions (the same
 * convention used by Codex), so injecting a fenced marker block keeps the
 * memory out of the command line and inside the file the agent already loads.
 *
 ...
 */
export function injectOpencodeInstructions(
  cwd: string,
  memoryText: string,
): InjectResult {
 ...
 */
export function removeOpencodeInstructions(
  cwd: string,
  created: boolean,
): void {
  const filePath = path.join(cwd, INSTRUCTIONS_REL)
