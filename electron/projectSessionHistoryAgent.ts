...
export function buildPrompt(input: {
  ...
  return truncateUtf8(
    [
      ...
      '- Do not preserve the same lesson in multiple categories. Keep one authoritative version in the narrowest useful category.',
      '- When sessions disagree on command syntax, prompt prefixes, or literal tokens, keep the corrected final form and remove the outdated variant.',
      '- Prefer code, config, build, and test paths over generic instruction documents. Only keep files such as AGENTS.md, README.md, or github/copilot-instructions.md when they define a durable repo-specific rule that is not better grounded elsewhere.',
